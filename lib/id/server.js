...
function _serverSocketStart(handler) {
    ...
    self._s.listen(self._opts.port, function(err){
        if ( err ) {
...
//      _debug(c.id+" > ",JSON.stringify(m));
    ...
    if ( m.command == "ask" ) {
        ...
        if ( !m.n || typeof m.n != "number" || m.n < 0 || m.n > MAXIDS )
        ...
        // Validation
        if ( !m.key || typeof m.key != "string" || m.key.match(/\r/) )
            return self._answer(c,"ask",{error: { code: "EINVKEY", description: "Invalid key" }});
